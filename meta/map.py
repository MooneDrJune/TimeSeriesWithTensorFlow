...
class Map(MetaStreamable):
    ...
    def step(self, inputs, states=None):
        ...
        op_value, next_state, init_state = self.operator(inputs_0, streamable=False)

        if states is None:
            extended_init_state = map_fn(
                init_state,
                [init_state],
        ...
        def apply_op(inputs_states):
            outputs, new_state, _ = self.operator(
                inputs=inputs_states[0], state=inputs_states[1], streamable=False
            )
            return outputs, new_state

        outputs, next_state = tf.map_fn(
            apply_op, (inputs, prev_state), dtype=(output_dtype, state_dtype)
        )
