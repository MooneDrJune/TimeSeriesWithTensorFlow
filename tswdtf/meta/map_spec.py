...
from tswdtf.meta.map import Map
...
class MapSpec(TestCase):
    def setUp(self):
        self.sheets = self.read_ods(self.from_test_res("map.ods", __file__))
    ...
    def test_map_simple(self):
        ...
        with tf.Session() as sess:
            output = sess.run(o, {x: sheet[["x0", "x1", "x2"]]})
    ...
    def test_map_multi_output(self):
        ...
        np.testing.assert_almost_equal(
            output[0], sheet[["y0", "y1", "y2"]].values, decimal=3
        )
        np.testing.assert_almost_equal(
            output[1], sheet[["y0", "y1", "y2"]].values, decimal=3
        )
    ...
    def test_map_multi_inputs(self):
        ...
        with tf.Session() as sess:
            output = sess.run(
                o, {x: sheet[["x0", "x1", "x2"]], y: sheet[["y0", "y1", "y2"]]}
            )
...
class MapFFillSpec(TestCase):
    ...
    def test_map_ffill(self):
        ...
        with tf.Session() as sess:
            output = sess.run(o, {values_ph: values})
