...
from tswdtf.meta.factory import Factory
...
class FactorySpec(TestCase):
    def setUp(self):
        self.sheets = self.read_ods(self.from_test_res("factory.ods", __file__))
    ...
    def test_factory_simple(self):
        ...
        np.testing.assert_almost_equal(output[0], sheet["Mul 3"].values, decimal=3)
        np.testing.assert_almost_equal(output[1], sheet["Mul 5"].values, decimal=3)
        np.testing.assert_almost_equal(output[2], sheet["Mul 10"].values, decimal=3)
