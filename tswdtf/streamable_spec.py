...
from tswdtf.streamable import Streamable, flatten
...
class MultiDimSpec(unittest.TestCase):
    def test_streamable_multidim(self):
        ...
        self.assertEqual(
            output.tolist(), [[0, 0], [2, 3], [4, 6], [6, 9], [8, 12], [10, 15]]
        )
...
class Complex(Streamable):
    def step(self, value1, value2, state1=None, state2=None):
        ...
        new_state2 = {"b": state2["b"] + tf.ones(2), "c": state2["c"] - 1}
...
class MultiNestedOperators(Streamable):
    ...
    def step(self, value, iteration=None, prev_state1=None, prev_state2=None):
        ...
        new_value1, next_state1, init_state1 = self.with_state(
            value, prev_state1, streamable=False
        )
        new_value2, next_state2, init_state2 = self.with_state(
            new_value1, prev_state2, streamable=False
        )
        return (
            new_value2 + 1,
            (iteration + 1, next_state1, next_state2),
            (iteration, init_state1, init_state2),
        )
...
class StreamableSpec(unittest.TestCase):
    ...
    def test_streamable_bad_input_state_operator(self):
        ...
        self.assertEqual(
            ...
            "Provided state and model state has different types (<dtype: 'int32'> != ()).",
        )
