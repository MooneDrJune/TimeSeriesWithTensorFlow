...
class Correlation(Streamable):
    ...
    def step(
        self, value1, value2, iteration=None, last_values1=None, last_values2=None
    ):
        ...
        return (
            correlation,
            (iteration + 1, new_values1, new_values2),
            (iteration, last_values1, last_values2),
        )
...
class CrossCorrelation(Streamable):
    ...
    def step(
        ...
        value2,
        iteration=None,
        prev_correlation_state=None,
        last_lag_buffer=None,
    ):
        """
        Value1 is leading
        """
        ...
        correlation, new_correlation_state = tf.cond(
            iteration < self.lag,
            lambda: (
                tf.zeros(tf.shape(value1), dtype=value1.dtype),
                prev_correlation_state,
            ),
        ...
        return (
            correlation,
            (iteration + 1, new_correlation_state, new_lag_buffer),
...
class AutoCorrelation(Streamable):
    ...
    def step(self, value, prev_corr_state=None):
        corr, corr_next_state, corr_init_state = self.cross_correlation(
            inputs=(value, value), state=prev_corr_state, streamable=False
        )
