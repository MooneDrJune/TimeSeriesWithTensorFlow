...
from tswdtf.common import shift
from tswdtf.streamable import Streamable


class GlobalMinimum(Streamable):
    def step(self, value, global_min=None, is_first_iteration=True):

        if global_min is None:
            global_min = tf.zeros(tf.shape(value), value.dtype)

        new_min = tf.cond(
            tf.logical_or(is_first_iteration, tf.less(value, global_min)),
            lambda: value,
            lambda: global_min,
        )
...
class GlobalMaximum(Streamable):
    def step(self, value, global_max=None, is_first_iteration=True):
        ...
        new_max = tf.cond(
            tf.logical_or(is_first_iteration, tf.greater(value, global_max)),
            lambda: value,
            lambda: global_max,
        )
