...
class MovingAverageConvergenceDivergence(Streamable):
    ...
    def step(
        ...
        histogram = self._diff(
            macd, signal, iteration, self.slow_period + self.macd_period - 2
        )

        return (
            (ema_slow, ema_fast, macd, signal, histogram),
            (iteration + 1, new_ema_slow_state, new_ema_fast_state, new_ema_macd_state),
            (iteration, ema_slow_init, ema_fast_init, ema_macd_init),
        )
