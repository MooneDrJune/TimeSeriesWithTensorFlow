...
class SharpeRatio(Streamable):
    ...
    def step(self, return_, risk_free_rate, last_adjusted_values=None, iteration=None):
        ...
        def compute_sharpe_ratio(adjusted_values):
            ...
            stddev = tf.sqrt(var)
            return tf.cond(tf.equal(stddev, 0.0), lambda: 0.0, lambda: mean / stddev)
        ...
        return (
            sharpe_ratio,
            (new_adjusted_values, iteration + 1),
