...
from tswdtf.finance.moving_average import ExponentialMovingAverage
from tswdtf.finance.moving_average import RollingMovingAverage
...
class SimpleMovingAveragesSpec(TestCase):
    ...
    def test_simple_moving_average(self):
        ...
        with tf.Session() as sess:
            output = sess.run([sma4_ts, sma10_ts], {prices: single_dim_ts["Close"]})

        np.testing.assert_almost_equal(
            output[0], single_dim_ts["SMA4"].values, decimal=3
        )
        np.testing.assert_almost_equal(
            output[1], single_dim_ts["SMA10"].values, decimal=3
        )
...
class ExponentialMovingAverageSpec(TestCase):
    ...
    def test_exponential_moving_average(self):
        ...
        np.testing.assert_almost_equal(output, s["EMA10"].values, decimal=3)
...
class RollingMovingAverageSpec(TestCase):
    ...
    def test_single_dim(self):
        ...
        with tf.Session() as sess:
            output = sess.run(rma10_ts, {values: s["Close"]})

        np.testing.assert_almost_equal(output, s["RMA10"].values, decimal=3)
