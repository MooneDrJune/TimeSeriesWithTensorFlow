...
class MACDSpec(TestCase):
    ...
    def test_single_dim(self):
        ...
        np.testing.assert_almost_equal(
            output_ts[0], expected_ema_slow.values, decimal=3
        )
        np.testing.assert_almost_equal(
            output_ts[1], expected_ema_fast.values, decimal=3
        )
        np.testing.assert_almost_equal(output_ts[2], expected_macd.values, decimal=3)
        np.testing.assert_almost_equal(output_ts[3], expected_signal.values, decimal=3)
        np.testing.assert_almost_equal(
            output_ts[4], expected_histogram.values, decimal=3
        )
