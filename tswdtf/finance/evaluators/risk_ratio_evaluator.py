...
class RiskRatioEvaluator(Streamable):
    def __init__(
        self, stop_factor, limit_factor, volatility_period=5, max_bet_duration=None
    ):
        super().__init__()
    ...
    def step(
        ...
        volatility, next_volatility_state, volatility_init = self.msd_op(
            inputs=close, state=previous_volatility_state, streamable=False
        )
        ...
        stop_hit = tf.logical_and(
            tf.logical_not(is_null), tf.less_equal(low, previous_stop_price)
        )
        limit_hit = tf.logical_and(
            tf.logical_not(is_null), tf.greater_equal(high, previous_limit_price)
        )
        ...
        buy_signal = tf.logical_and(is_null, tf.equal(signal, 1))
        ...
        return (
            (next_state[4], next_state[5], volatility, next_state[6]),
            (*next_state[0:6], next_volatility_state),
            (
                previous_stop_price,
                previous_limit_price,
                previous_buy_price,
