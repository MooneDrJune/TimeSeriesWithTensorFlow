...
from tswdtf.finance.returns import Return, LogarithmicReturn
...
class ReturnSpec(TestCase):
    ...
    def test_return_single_dim(self):
        ...
        with tf.Session() as sess:
            output = sess.run(return_ts, {prices: single_dim_ts["Prices"]})

        np.testing.assert_almost_equal(
            output, single_dim_ts["Return 5d"].values, decimal=4
        )
...
class LogarithmicReturnSpec(TestCase):
    def setUp(self):
        self.sheets = self.read_ods(
            self.from_test_res("logarithmic_returns.ods", __file__)
        )
    ...
    def test_log_return_single_dim(self):
        ...
        with tf.Session() as sess:
            output = sess.run(log_return_ts, {prices: single_dim_ts["Prices"]})

        np.testing.assert_almost_equal(
            output, single_dim_ts["Log Return 5d"].values, decimal=4
        )
