...
from tswdtf.common.automaton import Automaton
...
automaton = Automaton(
    ...
    init_state=s0,
    default_action=lambda x, y: tf.stack((-1, -1)),
)


class AutomatonSpec(unittest.TestCase):
    def _test_transition(self, from_s, values, to_s):
        x = automaton(inputs=values, state=from_s, streamable=False)
    ...
    def test_stream(self):
        ...
        a = automaton(inputs=(x, y), state=s0)
        with tf.Session() as sess:
            b = sess.run(a)
