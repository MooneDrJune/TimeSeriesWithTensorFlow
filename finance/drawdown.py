...
from tswdtf.common import filter_with_mask, map_consecutive_fn
...
class Drawdown(Streamable):
    def step(self, value, last_peak=None, last_periods_under_water=None):
        ...
        new_periods_under_water = tf.cond(
            tf.abs(new_peak - last_peak) > ERROR,
            lambda: 0,
            lambda: last_periods_under_water + 1,
        )
        drawdown = tf.cond(
            tf.abs(last_peak) < ERROR,
            lambda: 0.0,
...
def details(drawdown, drawdown_days):
    ...
    period_beginnings_mask = tf.map_fn(lambda x: tf.equal(x, 1), d, dtype=tf.bool)

    period_endings_mask = map_consecutive_fn(
        lambda x, y: tf.logical_and(tf.greater(y, 0), tf.equal(x, 0)), d
    )

    period_beginnings = filter_with_mask(period_beginnings_mask, idx)
    period_endings = filter_with_mask(period_endings_mask, idx)

    bounds = tf.transpose(tf.stack([period_beginnings, period_endings])) - 1

    return tf.map_fn(compute_avg, bounds, dtype=(tf.float32, tf.int32))
