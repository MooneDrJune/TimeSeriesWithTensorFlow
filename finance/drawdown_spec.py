...
from tswdtf.finance.drawdown import Drawdown, avg
...
class AverageTrueRangeSpec(TestCase):
    def setUp(self):
        self.input_ts = self.read_csv(
            self.from_test_res("drawdown.csv", __file__)
        ).astype("float32")
    ...
    def test_drawdown(self):
        ...
        np.testing.assert_almost_equal(
            output[0], self.input_ts["Drawdown"].values, decimal=3
        )
        np.testing.assert_almost_equal(
            output[1], self.input_ts["Time Under Water"].values, decimal=3
        )
    ...
    def test_average_drawdown(self):
        ...
        with tf.Session() as sess:
            output = sess.run(
                [mean_drawdown, mean_drawdown_days], {values: actual_values}
            )
