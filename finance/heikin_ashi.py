...
class HeikinAshi(Streamable):
    ...
    def step(
        ...
        ha_close_price = tf.cond(
            tf.equal(prev_open_price, 0),
            lambda: close_price,
            lambda: tf.reduce_sum([close_price, low_price, high_price, open_price]) / 4,
        )

        ha_open_price = tf.cond(
            tf.equal(prev_open_price, 0),
            lambda: open_price,
            lambda: (prev_open_price + prev_close_price) / 2,
        )

        ha_low_price = tf.cond(
            tf.equal(prev_open_price, 0),
            lambda: low_price,
            lambda: tf.reduce_min(tf.stack([low_price, ha_close_price, ha_open_price])),
        )

        ha_high_price = tf.cond(
            tf.equal(prev_open_price, 0),
            lambda: high_price,
