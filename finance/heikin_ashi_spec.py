...
from tswdtf.finance.heikin_ashi import HeikinAshi
from tswdtf.tests import TestCase


class HeikinAshiSpec(TestCase):
    def setUp(self):
        self.input_ts = self.read_ods(self.from_test_res("heikin_ashi.ods", __file__))[
            "Sheet1"
        ]
    ...
    def test_heikin_ashi(self):
        ...
        np.testing.assert_almost_equal(
            output[0], self.input_ts["ha_open"].values, decimal=3
        )
        np.testing.assert_almost_equal(
            output[1], self.input_ts["ha_high"].values, decimal=3
        )
        np.testing.assert_almost_equal(
            output[2], self.input_ts["ha_low"].values, decimal=3
        )
        np.testing.assert_almost_equal(
            output[3], self.input_ts["ha_close"].values, decimal=3
        )
