...
from tswdtf.finance.evaluators.risk_ratio_evaluator import RiskRatioEvaluator
...
class RiskRatioEvaluatorSpec(TestCase):
    ...
    def test_risk_ratio_evaluator_without_bet_duration(self):
        ...
        np.testing.assert_almost_equal(
            output[0], input_ts["successful"].values, decimal=3
        )
        np.testing.assert_almost_equal(output[1], input_ts["total"].values, decimal=3)
        np.testing.assert_almost_equal(output[3], input_ts["signal"].values, decimal=3)

    def test_risk_ratio_evaluator_with_bet_duration(self):
        evaluator = RiskRatioEvaluator(
            stop_factor=2.0, limit_factor=4.0, max_bet_duration=5
        )
        close_prices = tf.placeholder(tf.float32)
        low_prices = tf.placeholder(tf.float32)
        high_prices = tf.placeholder(tf.float32)
